...
func TestTokenListContainsValue(t *testing.T) {
	for _, tt := range tokenListContainsValueTests {
		h := []string{tt.value}
		ok := tokenListContainsValue(h, "websocket")
		if ok != tt.ok {
...
func TestParseExtensions(t *testing.T) {
	for _, tt := range parseExtensionTests {
		req := protocol.NewRequest("", "", nil)
		req.SetHeader("Sec-WebSocket-Extensions", "permessage-deflate; client_max_window_bits")
		extensions := parseExtensions(protocolReqHeaderValueByKey(&req.Header, "Sec-WebSocket-Extensions"))
